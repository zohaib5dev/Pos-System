<!-- Flash Message Container - Handles both session flashes and Livewire notifications -->
<div 
    x-data="flashMessageHandler()"
    @notify.window="handleLivewireNotification($event.detail)"
>
    <!-- Session-based flash message (for redirects) -->
    <template x-if="showSessionFlash">
        <div class="position-fixed" style="top: 20px; right: 20px; width: 40%; min-width: 250px; max-width: 450px; z-index: 9999;">
            <div class="flash-message alert" 
                 :class="sessionFlashType === 'success' ? 'alert-success' : 'alert-danger'"
                 role="alert"
                 x-data="{ progress: 100 }"
                 x-init="startProgress()"
                 x-show="showSessionFlash"
                ...
                <div class="d-flex align-items-center">
                    <div class="mr-3">
                        <i class="fas fa-2x" 
                           :class="sessionFlashType === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'"></i>
                    </div>
                    <div class="flex-grow-1" style="word-break: break-word;">
                        <strong x-text="sessionFlashType === 'success' ? 'Success!' : 'Error!'"></strong>
                        <p class="mb-0 small" x-text="sessionFlashMessage"></p>
                    </div>
                    <button type="button" class="close ml-3" @click="showSessionFlash = false" aria-label="Close">
                ...
                <div class="progress mt-2" style="height: 3px;">
                    <div class="progress-bar" 
                         :class="sessionFlashType === 'success' ? 'bg-success' : 'bg-danger'"
                         role="progressbar" 
    ...
    <template x-for="(notification, index) in livewireNotifications" :key="index">
        <div class="position-fixed" style="top: 20px; right: 20px; width: 40%; min-width: 250px; max-width: 450px; z-index: 9999;">
            <div class="flash-message alert" 
                 :class="notification.type === 'success' ? 'alert-success' : 'alert-danger'"
                 role="alert"
                 x-data="{ progress: 100 }"
                 x-init="startNotificationProgress(notification.id)"
                ...
                <div class="d-flex align-items-center">
                    <div class="mr-3">
                        <i class="fas fa-2x" 
                           :class="notification.type === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'"></i>
                    </div>
                    <div class="flex-grow-1" style="word-break: break-word;">
                        <strong x-text="notification.type === 'success' ? 'Success!' : 'Error!'"></strong>
                        <p class="mb-0 small" x-text="notification.message"></p>
                    </div>
                    <button type="button" class="close ml-3" @click="removeNotification(notification.id)" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
<!-- Session-based flash message fallback (for initial page load) -->
@if(session()->has('message'))
<div class="position-fixed session-flash-fallback" style="top: 20px; right: 20px; width: 40%; min-width: 250px; max-width: 450px; z-index: 9999;">
...
<style>
    ...
    .flash-message.alert-success i {
        color: rgba(255, 255, 255, 0.95) !important;
    }

    /* Error icon color */
    .flash-message.alert-danger .fa-exclamation-circle,
    .flash-message.alert-danger i {
        color: rgba(255, 255, 255, 0.95) !important;
    }
    ...
    /* Hide session flash fallback when Alpine loads */
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('flashMessageHandler', () => ({
            // Session flash handling
            showSessionFlash: {{ session()->has('message') ? 'true' : 'false' }},
            sessionFlashMessage: '{{ session('message', '') }}',
            sessionFlashType: '{{ session('message_type', 'success') }}',
            
            // Livewire notifications
            livewireNotifications: [],
            nextId: 1,
            
            init() {
                // Clear session flash after 3 seconds
                if (this.showSessionFlash) {
                    setTimeout(() => {
                        this.showSessionFlash = false;
                    }, 3000);
                }
            },
            ...
            startProgress() {
                const interval = setInterval(() => {
                    if (this.$data.progress > 0) {
                        this.$data.progress = this.$data.progress - (100/30);
                    } else {
            ...
            handleLivewireNotification(detail) {
                const notification = {
                    id: this.nextId++,
                    message: detail.message || detail[0]?.message || 'Notification',
                    type: detail.type || detail[0]?.type || 'success',
                    show: true,
                    progress: 100
                };
                
                this.livewireNotifications.push(notification);
            ...
            startNotificationProgress(notificationId) {
                const notification = this.livewireNotifications.find(n => n.id === notificationId);
                if (!notification) return;
                
                const interval = setInterval(() => {
                    const currentNotification = this.livewireNotifications.find(n => n.id === notificationId);
                    if (currentNotification && currentNotification.progress > 0) {
                        currentNotification.progress = currentNotification.progress - (100/30);
                    } else {
                        clearInterval(interval);
                    }
                }, 100);
            ...
            removeNotification(id) {
                const index = this.livewireNotifications.findIndex(n => n.id === id);
                if (index !== -1) {
                    this.livewireNotifications[index].show = false;
                    setTimeout(() => {
                        this.livewireNotifications = this.livewireNotifications.filter(n => n.id !== id);
                    }, 500);
                }
            }
        }));
